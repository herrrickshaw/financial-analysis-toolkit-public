...
SECTOR_PROFILES: Dict[str, SectorProfile] = {
    ...
    "utility": SectorProfile("Regulated utilities / electric & gas", "low", 8, 0.35, -0.15, 0.15,
        "Tuned on Duke Energy (DUK) and five real regulated-utility peers (SO, AEP, D, EXC, XEL), FY2025 real "
        "prices/EDGAR data. Deliberately the LOWEST unlevered beta of any sector profiled so far (0.35, vs. "
        "REIT's 0.90 and every commodity/financial sector's 1.05+) — regulated, cost-of-service revenue is the "
        "lowest-systematic-risk cash flow this module has tuned against. But 'low cyclicality' turned out to mean "
        "something different here than for software: `trend_diagnostics()` fires the SAME 'strong improving "
        "secular trend' CAUTION on DUK's real OI margin (23.4% FY2019 -> 27.2% FY2025, r=0.75, `finmodel cycle "
        "data/edgar/DUK.json --sector utility`) that it fired for Cisco/Salesforce (software) and Travelers "
        "(insurance) — a THIRD, independent sector confirming the trend-guard generalizes — but the real cause is "
        "sector-specific: a regulated utility's allowed revenue grows with its rate base, and DUK's rate base has "
        "been growing (real capex has run 39.6%-45.7% of revenue every single year FY2019-2025, verified), so a "
        "trailing-median 'normalization' would misread real, sustained rate-base growth as a cyclical peak to "
        "fade, the same mistake it would make on Salesforce's margin expansion. "
        "\n\n"
        "The real, sector-defining finding: a standard unlevered DCF technically runs for a utility (unlike "
        "banking/REIT/insurance, no residual-income/DDM substitute is required), but verified on DUK's own real "
        "FY2019-2025 history that unlevered free cash flow (NOPAT + D&A - capex, before working capital) was "
        "NEGATIVE in 2 of those 7 years and never exceeded ~$1.3B against $87B of debt and a $99B market cap — "
        "because real capex ran a persistent ~1.8-2.1x real D&A every year, not a temporary supercycle like the "
        "semiconductor check's (which tapered its capex assumption back down over the forecast). For a utility "
        "the opposite fix applies: DON'T taper elevated capex toward a lower 'normal' level, because the elevated "
        "level *is* the normal level — rate-base growth capex is the entire regulated-return mechanism, not a "
        "temporary drag to fade. The practical consequence, verified in the DCF build: with near-term UFCF this "
        "thin, terminal value carries the large majority of enterprise value, functionally converging a "
        "'standard' unlevered DCF toward the same perpetuity-dominated shape as a REIT's dividend discount model, "
        "without actually needing to swap engines the way REITs/banks/insurers did — with one sharp, real "
        "consequence: verified that a GENERIC 2.5% terminal growth rate (this toolkit's usual default) produces a "
        "NONSENSICAL NEGATIVE implied equity value for DUK, a real, solvent, investment-grade, dividend-paying "
        "company, because it silently assumes DUK's own real, currently-disclosed $103B 2026-2030 capital plan "
        "(DUK's own guidance: 5%-7% long-run EPS growth off 9.6% rate-base growth) stops mattering the instant "
        "the 5-year forecast window ends. The fix is to anchor terminal growth to the company's own real, "
        "disclosed guidance instead of a generic assumption — moderated safely below WACC, since a Gordon-growth "
        "perpetuity is mathematically undefined once growth reaches the discount rate — which is what makes this "
        "sector uniquely sensitive to the terminal-growth choice: DUK's own WACC (6.19%, the lowest of any sector "
        "profiled) leaves very little headroom between a defensible long-run growth rate and the rate at which "
        "the perpetuity formula breaks down. "
        "\n\n"
        "A second, real, verified caveat this check surfaced for `finmodel.wacc.synthetic_rating()`: DUK's real "
        "FY2025 interest coverage (EBIT $8,626M / interest expense $3,634M = 2.37x) maps to a synthetic 'BB+' "
        "(sub-investment-grade) rating on Damodaran's large-firm coverage table — but DUK's REAL rating, as of "
        "Feb 2026, is investment-grade (BBB from S&P, Baa2 from Moody's, both stable outlook). The coverage table "
        "was built for unregulated industrials; a regulated utility is ALLOWED to run structurally thin coverage "
        "because rate-of-return regulation makes debt-service recovery through rates close to guaranteed, "
        "something the ratio alone can't see. Same tool, same low-coverage signal as the airline check's "
        "genuinely distressed ALK — opposite real-world meaning, a real reason to sanity-check a coverage-based "
        "synthetic rating against the filer's ACTUAL published rating before using it, especially for a "
        "regulated entity. "
        "\n\n"
        "A third, independent real mechanism (from peer Xcel Energy, XEL, not the target): XEL's own ROE "
        "(net_income/equity, the same override banking/insurance use) held a genuinely tight 10.1%-10.4% band for "
        "seven straight years (FY2018-2024) before dropping to 8.5% in FY2025 — `trend_diagnostics()` correctly "
        "flags this whole 8-year window as a 'declining' trend (r=-0.70), but the real cause isn't earnings "
        "deterioration: XEL closed a ~$1.18B forward common-stock offering in November 2024 and disclosed a "
        "further $4.3B equity distribution program funding its multi-year capital plan, mechanically diluting "
        "ROE in the issuance year before the new equity has earned a full year's return — a real, utility-"
        "specific 'growth-funding dilution' mechanism, distinct from every other sector's cyclicality driver "
        "(commodity price, credit losses, rate-sensitivity, demand shock, catastrophe losses, the silicon cycle) "
        "this module has been tuned against. "
        "\n\n"
        "Two real, narrower EDGAR-extraction findings from building the comps set: (1) Xcel Energy's own "
        "consolidated top line moved onto the utility-industry-specific `RegulatedAndUnregulatedOperatingRevenue` "
        "XBRL tag starting FY2022 (its plain `Revenues` tag has zero entries from FY2022 onward) — now a real "
        "fallback in `finmodel.edgar`'s revenue tag list, a true alternative rather than an additive component, "
        "since it's the filer's own single undisaggregated total. (2) SEC's own `companyfacts` API returns a "
        "stale `entityName` ('The Southern Company') for CIK 1004155, while the authoritative `submissions` API "
        "correctly shows its real current name (Southern Co Gas, formerly AGL Resources Inc, renamed on the real "
        "2016-07-11 merger-close date) — a genuine, verified SEC API inconsistency, not a bug in this toolkit; "
        "don't trust `companyfacts`' `entityName` field alone for entity identification, cross-check against "
        "`submissions` or EDGAR's own company search."),
    "pharma": SectorProfile("Pharmaceuticals", "moderate", 8, 0.75, -0.20, 0.20,
        "Tuned on AbbVie (ABBV) and four real large-cap peers (PFE, MRK, BMY, LLY), FY2025 real prices/EDGAR "
        "data. Like airlines/semiconductors/utilities, the default operating_income/revenue field needs no "
        "OVERRIDE (the standard EV-based comps framework applies fine) — but it does need a real ADJUSTMENT "
        "before its year-to-year history can be trusted, a third distinct pattern alongside 'wrong field "
        "entirely' (banking/REIT/insurance) and 'right field, no fix needed' (airlines/semiconductors/utilities). "
        "The real cause: AbbVie's own GAAP-mandated (ASC 730-10-25-2c) acquired-in-process-R&D write-offs — "
        "expensed immediately when acquired via an asset acquisition with no alternative future use — ran "
        "$0.7B-$5.0B/year FY2020-2025 (real, verified against SEC's live XBRL data), with the real FY2025 charge "
        "($5.0B, from the 2024-closed ImmunoGen and Cerevel Therapeutics deals) alone compressing reported "
        "operating margin by 8.2 percentage points (24.6% -> 32.8% once added back). On the RAW field, "
        "`trend_diagnostics()` shows a misleadingly negative-leaning correlation (r=-0.33) driven by this "
        "single FY2024 charge-year trough; on the ADJUSTED field (operating_income + the real, separately-"
        "tagged acquired_iprd_writeoff — precomputed as `adjusted_operating_income` in data/edgar/ABBV.json) it "
        "resolves to a genuinely weak trend (r=-0.09) — meaning the GENERIC `dcf_scenarios_from_history()` "
        "trailing min/median/max is the right tool here, unlike the DUK/TRV/CSCO checks' real secular trends, "
        "PROVIDED the input is fixed first. This is mechanically different from the semiconductor check's R&D "
        "capitalization (finmodel.rd_capitalization): that module amortizes a real multi-year asset (ORGANIC "
        "R&D) that GAAP expenses too early; an acquired-IPR&D write-off is a real one-time ACQUISITION cost GAAP "
        "correctly expenses all at once — normalizing it means adding it back for comps purposes (like a "
        "restructuring charge), not capitalizing/amortizing it forward. The real, sector-defining finding, "
        "separate from the above: AbbVie's own genuine Humira patent cliff (US exclusivity lost 2023; real "
        "global Humira revenue $21.2B FY2022 -> $8.99B FY2024, AbbVie's own disclosed figures) never dragged "
        "AbbVie's TOTAL revenue down by more than 6.4% in any year and fully recovered within two, because its "
        "own real, disclosed Skyrizi+Rinvoq replacement franchise grew from ~$16B to $25.9B in a single year "
        "(2027 guidance raised to a combined $31B) — a real, generalizable lesson that a naive 'patent cliff "
        "sinks the company' DCF assumption is right for a single-product biotech (see docs/FOOTBALL_FIELD_ABBV.md's "
        "Seagen precedent, genuinely pre-profitability with no such offset) and WRONG for a diversified major "
        "pharma with a real, funded pipeline. Also found: a real EDGAR extraction gap (AbbVie's and Merck's own "
        "\"da\" figure silently dropped real, material intangible amortization because neither filer ever "
        "populates a combined D&A tag — now a real additive fix in `finmodel.edgar`), and an unrelated real "
        "`market_data` warehouse bug (duplicate, unpurged OHLC batches specific to the ABBV ticker)."),
