"""Consolidates this toolkit's India investment-incentive research -- state schemes, central schemes,
land-cost benchmarks, and the worked 12-state matrix -- into a single, multi-sheet Excel workbook: the same
facts as `docs/INDIA_STATE_INVESTMENT_INCENTIVES.md`, `docs/INDIA_CENTRAL_INVESTMENT_INCENTIVES.md`,
`docs/INDIA_INDUSTRIAL_LAND_COST_BENCHMARKS.md`, and `docs/INDIA_STATE_SECTOR_INCENTIVE_MATRIX.md`, in a
spreadsheet a non-technical reader can filter and sort directly rather than reading four markdown files.
...
README_LINES = [
    "India Investment Incentive & Bankability Workbook -- generated by finmodel (financial-analysis-toolkit)",
    "",
    "Sheets:",
...
def build_workbook(path: str) -> str:
    ...
    return str(write_record_tables(path, sheets, readme_lines=README_LINES))
