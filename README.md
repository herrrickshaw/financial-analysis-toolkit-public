...
pytest -q                         # 84 tests; the LibreOffice recalc test auto-skips if soffice is absent
```
...
- `finmodel.sectors` (`finmodel cycle <edgar-json> --sector steel|oil_gas|software|banking [--field --revenue-field]`) — the sector-tuning module all four checks above validate: `cycle_diagnostics()` flags whether a year is a cycle trough/peak/normal relative to its own trailing history (confirmed automatically on STLD FY2025 = trough, PXD FY2022 = peak, CSCO FY2026 = near normal, USB's FY2020 COVID reserve build = trough — the exact readings the docs above found by hand), `dcf_scenarios_from_history()` derives data-driven bear/base/blue-sky scenario targets instead of a hand-picked one, `normalized_comps_metrics()` swaps a raw LTM EBITDA for a through-cycle median, and `trend_diagnostics()` catches the case none of the above should be trusted on: a genuine secular trend (Salesforce's growth-to-profitability shift, ~2%→20% margin over 8 years) rather than mean-reverting cyclicality, where trailing-median normalization would badly misread a sustainable margin as a misleading cyclical extreme. `--field`/`--revenue-field` override the default operating-income/revenue margin with any other ratio — ROE (`net_income`/`equity`) for a bank, whose standard revenue/EBIT tags the banking check proved are meaningless. Steel, oil & gas, enterprise software and banking are tuned against real multi-year data so far; everything else uses a clearly-labelled generic default.
...
scripts/           comps_validation.py, football_field_stld.py, football_field_cvx.py, football_field_csco.py, football_field_usb.py, ma_case_study.py (regenerate the real-data docs)
