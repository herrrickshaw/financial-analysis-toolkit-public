...
columns as its sheet's header), and that the 27-State Matrix sheet's numbers are computed live and match
...
def test_matrix_rows_match_the_underlying_modules_directly():
    ...
    # 27 state rows + 1 TOTAL row
    assert len(matrix_rows["rows"]) == 28
    ...
    assert total_row[0] == "TOTAL (27 states)"
...
def test_build_workbook_writes_all_sheets(tmp_path):
    ...
    assert wb.sheetnames == ["README", "State Incentives", "Central Incentives", "Land Cost Benchmarks",
                             "27-State Matrix", "Tax Regime Comparison", "Financing Effects"]
    ...
    ws = wb["27-State Matrix"]
    assert ws.cell(row=1, column=1).value == "State"
    assert ws.cell(row=29, column=1).value == "TOTAL (27 states)"
